  ...
  if (isset($image) && (count($image['name']) > 0) && !empty($image['name'][0])) {
    for ($i=0; $i < count($image['name']); $i++) { 
      ...
      if ($image_size > 2097152 || $image_size < 2) {
       header("location:../general_image_add.php?msg=6");
       die();
     }
     ...
     if( $ext=='jpg' || $ext=='jpeg' || $ext=='png' || $ext=='bmp' || $ext=='gif' ){
         ...
         echo($ext);
         die();
       header("location:../general_image_add.php?msg=7");
       die();
     }
   }
 }else{
  header("location:../general_image_add.php?msg=8");
  die();
}
  ...
  for ($i=0; $i < count($image['name']); $i++) { 
    ...
    if (!empty($image['name'][$i])) {
      ...
      if( $ext=='jpg' || $ext=='jpeg' || $ext=='png' || $ext=='bmp' || $ext=='gif' ){
        ...
        $path = "../../uploads/general/".$image_name ;
        move_uploaded_file($image_tmp_name,$path);

        include_once("ak_php_img_lib_1.0.php");
        $thumb_path = "../../uploads/general/thumb/".$image_name;
        $wmax = 300;
        $hmax = 200;
        ak_img_resize($path,$thumb_path, $wmax, $hmax, $ext);
        date_default_timezone_set("Asia/Kolkata");
        $date = date('Y-m-d');
        $sql="INSERT INTO `general_gallery` (`image`, `uploaded_date`) VALUES ('$image_name','$date')";
...
header("location:../general_image_add.php?msg=1"); 
}else{
  header("location:../general_image_add.php?msg=8");
}
}else{
  header("location:../general_image_add.php?msg=3");
}
