...
if (isset($_POST['submit']) && isset($_POST['mobile']) && !empty($_POST['mobile'])) {
    ...
    if ($res_user = $connect->query($sql_user)) {
        if ($res_user->num_rows > 0) {
            ...
            $sql_insert = "INSERT INTO `user_details`(`user_id`, `name`, `prefered_name`, `father_name`, `mother_name`, `category`, `gh_details`, `gh_pin`, `gh_dist`, `gh_state`, `phy_status`, `phy_doctor`, `phy_exp`, `phy_phone`, `phy_category`, `guru_name`, `guru_address`, `guru_pin`, `guru_dist`, `age`, `dob`, `mobile`,`survay_sign`,`date`) VALUES ('$user_id','$name','$prefered_name','$father_name','$mother_name','$catgegory','$gh_details','$gh_pin','$gh_dist','$gh_state','$phy_status','$phy_doctor','$phy_exp','$phy_phone','$phy_category','$guru_name','$guru_address','$guru_pin','$guru_dist','$age','$dob','$user_mobile','$surbay_sign','$date')";
            if ($res_insert = $connect->query($sql_insert)) {
                $id = $connect->insert_id;
                $address_id = address_insert($connect,$user_id,$p_address,$p_dist,$p_pin);

                if ($address_id != false) {
                    $update_address_p = "UPDATE `user_details` SET `p_address_id` = '$address_id' WHERE `id`='$id'";
                    $connect->query($update_address_p);
                }
                $address_id_r = address_insert($connect,$user_id,$r_address,$r_dist,$r_pin);
                if ($address_id_r != false) {
                    $update_address_r = "UPDATE `user_details` SET `r_address_id` = '$address_id_r' WHERE `id`='$id'";
                ...
                $sql_documents = "INSERT INTO `documents`(`user_id`, `birth`, `voter_id`, `pan_card`, `aadhar_card`, `ration_card`, `bank_pass`, `mnrg_card`, `passport`, `hslc_cert`, `hs_cert`, `graduation_cert`, `name_chng`) VALUES ('$user_id','$birth_name','$voter_name','$pan_name','$aadhar_name','$ration_name','$bank_name','$mnrg_name','$pass_name','$hslc_name','$hs_name','$graduation_name','$name_name')";
                ...
                if (isset($other_q_status)) {
                    if ($other_q_status == 'yes') {
                        ...
                        $sql_other_q_update ="UPDATE `user_details` SET `other_q_status` = '$other_q_status', `other_q_name` = '$other_q_name', `other_q_year` = '$other_q_year', `other_q_file` = '$other_q_image'  WHERE `id`='$id'";
                ...
                if (isset($q_name) && !empty($q_name) && count($q_name) > 0) {
                    for ($i=0; $i < count($q_name); $i++) { 
                        ...
                        if (isset($q_file['name'][$i]) && !empty($q_file['name'][$i])) {
                        ...
                        $sql_q_insert = "INSERT INTO `qualification`(`user_id`, `name_school`, `passing_year`, `file`) VALUES ('$user_id','$q_names','$q_years','$q_image_names')";
                ...
                $sql_income ="UPDATE `user_details` SET `income_category` = '$income', `income_file` = '$income_image'  WHERE `id`='$id'";
                ...
                if (isset($bullying_status)) {
                    if ($bullying_status == 'yes') {
                        ...
                        $sql_other_q_update ="UPDATE `user_details` SET `bullying_status` = '$bullying_status', `bullying_file` = '$bullying_image'  WHERE `id`='$id'";
                        $connect->query($sql_other_q_update);
                ...
                if (isset($police_status)) {
                    if ($police_status == 'yes') {
                        ...
                        $sql_other_q_update ="UPDATE `user_details` SET `police_status` = '$police_status', `police_file` = '$police_image'  WHERE `id`='$id'";
                        $connect->query($sql_other_q_update);
                ...
                if (isset($disc_status)) {
                    if ($disc_status == 'yes') {
                        ...
                        $sql_other_q_update ="UPDATE `user_details` SET `disc_status` = '$disc_status', `disc_file` = '$disc_image'  WHERE `id`='$id'";
                        $connect->query($sql_other_q_update);
                ...
                if (isset($family_status)) {
                    if ($family_status == 'yes') {
                        ...
                        $sql_other_q_update ="UPDATE `user_details` SET `family_status` = '$family_status', `family_file` = '$family_image'  WHERE `id`='$id'";
                        $connect->query($sql_other_q_update);
...
function address_insert($connect,$user_id,$address,$dist,$pin)
{
    $sql = "INSERT INTO `address`(`user_id`, `dist`, `pin`, `address`) VALUES ('$user_id','$dist','$pin','$address')";
